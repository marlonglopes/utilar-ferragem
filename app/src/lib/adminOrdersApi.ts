...
const MOCK: AdminOrder[] = [
  ...
  },
  {
    ...
    subtotal: 34.5,
    total: 34.5,
    customerName: 'Balcão — venda avulsa',
    createdAt: '2026-07-28T09:10:00Z',
    paidAt: '2026-07-28T09:10:00Z',
  },
  {
...
function mockOrders(q: AdminOrderQuery): AdminOrdersPage {
  let rows = MOCK
  if (q.status === 'active')
    rows = rows.filter((o) => ['pending_payment', 'paid', 'picking', 'shipped'].includes(o.status))
  ...
  if (q.q) {
    ...
    rows = rows.filter(
      (o) => (o.customerName ?? '').toLowerCase().includes(needle) || o.id.includes(needle)
    )
  }
