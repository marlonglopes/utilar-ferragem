...
- **A1 — 🟢 DEFINITIVO IMPLEMENTADO (dormente até as chaves entrarem).** `pkg/servicetoken` emite e verifica; `role=service` **só** vale assinado com o segredo/chave de serviço, e um token de usuário com essa claim é rejeitado em todos os serviços. **A Alice não recebe segredo/chave de serviço** — é esse o ponto. **Topologia real:** o **order-service é o ÚNICO emissor** (assina); **catalog, auth E payment são verificadores** (⚠️ o payment VERIFICA — a afirmação antiga "payment não" estava errada). **Assinatura assimétrica Ed25519 pronta e ligada** (`Signer`/`Verifier`): o order assina com `SERVICE_JWT_PRIVATE_KEY`, os verificadores usam só `SERVICE_JWT_PUBLIC_KEY` → um verificador comprometido não emite mais token de serviço. O `Verifier` é **dual** (aceita Ed25519 e HS256 legado), então a migração é sem downtime. **Para ativar:** gere um par com `servicetoken.GenerateKeyPair()`, ponha a PRIVADA no order e a PÚBLICA nos verificadores; hoje, sem as chaves, roda em HS256 como antes. Boot é fail-closed fora de `DEV_MODE`.
